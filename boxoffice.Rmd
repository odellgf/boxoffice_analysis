...
```{r average_boxoffice_top15, cache=F}
g.d2
```
...
```{r average_RT_score_top15, cache=F}
g.d3
```
...
```{r average_theaters_top15, cache=F}
g.d4
```
...
ggplot(boxoffice, aes(studio.status.threecat, dom.box.mil)) +
  ...
  ylab("Studio status") +
  xlab("Box office") +
  ggtitle("Boxplots of box office performance (millions, USD)\nfor each studio status category")
```
...
```{r month_plots_num_theaters, cache = F}
g.m1 + g.m5
```
...
```{r month_plots_RT_boxoffice, cache = F}
(g.m3 + g.m4) / g.m2
```
...
m2.vif <- ols_vif_tol(m8d.2.1.i) %>% as_tibble()
